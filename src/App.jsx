...
const NAV = [
  ...
  { id: 'd', label: 'Design System', children: [
    ...
    ...componentIds.map(cid => ({ id: 'c-' + cid, label: componentDocs[cid].name, isComponent: true })),
...
export default function App() {
  ...
  const { getCSSVars, brand, mode } = useTheme();
  ...
  }, [getCSSVars, brand, mode]);

  const toggleSection = (id) => {
    setExpanded(prev => prev.includes(id) ? prev.filter(s => s !== id) : [...prev, id]);
  };

  let currentSection = null, currentChild = null;
  NAV.forEach(s => s.children.forEach(c => { if (c.id === page) { currentSection = s; currentChild = c; } }));
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: 'var(--bg)', color: 'var(--text)', fontFamily: 'var(--font)' }}>
      {/* SIDEBAR */}
      <aside style={{ width: sidebarOpen ? 240 : 0, flexShrink: 0, transition: 'width 200ms ease', overflow: 'hidden', borderRight: '1px solid var(--border)', background: 'var(--bg1)' }}>
        <div style={{ width: 240, height: '100vh', display: 'flex', flexDirection: 'column', position: 'sticky', top: 0 }}>
          <div style={{ padding: '16px 18px', borderBottom: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 'var(--radius)', background: 'linear-gradient(135deg, var(--accent), var(--accent-pressed))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, color: '#fff', fontFamily: 'var(--font-display)' }}>W</div>
              <div>
            ...
            {NAV.map(section => {
              ...
              return (
                <div key={section.id} style={{ marginBottom: 4 }}>
                  <button onClick={() => toggleSection(section.id)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 8px', borderRadius: 'var(--radius-sm)', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text3)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font)' }}>
                    {section.label}
                    <span style={{ fontSize: 9, transition: 'transform 200ms', transform: isOpen ? 'rotate(0)' : 'rotate(-90deg)' }}>▾</span>
                  </button>
                  {isOpen && <div style={{ marginLeft: 4 }}>
                    {section.children.map(child => {
                      const isActive = page === child.id;
                      return (
                        <button key={child.id} onClick={() => setPage(child.id)} style={{
                          display: 'block', width: '100%', textAlign: 'left',
                          padding: '5px 10px', paddingLeft: child.isComponent ? 22 : 10,
                          borderRadius: 'var(--radius-sm)', fontSize: 13, fontFamily: 'var(--font)',
                          color: isActive ? 'var(--accent)' : child.isComponent ? 'var(--text3)' : 'var(--text2)',
                          background: isActive ? 'var(--accent-bg)' : 'transparent',
                          border: 'none', cursor: 'pointer', marginBottom: 1, transition: 'all 120ms',
                        }}
          ...
          <div style={{ padding: '12px 18px', borderTop: '1px solid var(--border)', fontSize: 10, color: 'var(--text3)', fontFamily: 'var(--font-mono)' }}>WA Technology © 2026</div>
        </div>
      </aside>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        <header style={{
          ...
          backgroundColor: 'color-mix(in srgb, var(--bg), transparent 10%)',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button onClick={() => setSidebarOpen(!sidebarOpen)} style={{ width: 28, height: 28, borderRadius: 'var(--radius-sm)', background: 'var(--bg2)', border: '1px solid var(--border)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text3)', fontSize: 11, transition: 'all 120ms' }}
        ...
        <div className="fade-in" key={`${page}-${brand}-${mode}`} style={{ padding: '36px 44px', flex: 1 }}>
          {content}
        </div>
      </main>
    </div>
